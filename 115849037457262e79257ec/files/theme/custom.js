//Edits made by Rahul Krishnan
//With special help from Noah Finer
...
jQuery(function($) {
  ...
  $.fn.fadeBanner = function($el, scrollClass) {
    if((this.offset().top < $(window).scrollTop()-500) && !$el.hasClass(scrollClass)) { //changed the height, so transition happens 325 px lower
      $el.addClass(scrollClass);
    } else if((this.offset().top >= $(window).scrollTop()-500) && $el.hasClass(scrollClass)) { //changed the height, so transition happens 325 px lower
      $el.removeClass(scrollClass);
    }
  }
    ...
    _attachEvents: function() {
      ...
      if($('body').hasClass('page-has-banner')) {
        $(window).on('scroll', function(){
          ...
          $('.main-wrap').checkElementPositioning($('body.page-has-banner'), $('.birdseye-header'), 'affix');
		  //adding it
		  $('.main-wrap').checkElementPositioning($('.logo'), $('.birdseye-header'), 'affix');
